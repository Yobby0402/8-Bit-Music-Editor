...
class PropertyPanelWidget(QWidget):
    ...
    def on_filter_enabled_changed(self, enabled: bool):
        """滤波器启用状态改变"""
        if self.current_track and self.current_track.filter_params:
            self.current_track.filter_params.enabled = enabled
            self.property_changed.emit(self.current_note, self.current_track)
    ...
    def on_filter_type_changed(self, index: int):
        ...
        if self.current_track and self.current_track.filter_params:
            ...
            self.current_track.filter_params.filter_type = filter_type_map.get(index, FilterType.LOWPASS)
            self.property_changed.emit(self.current_note, self.current_track)
    
    def on_filter_params_changed(self):
        """滤波器参数改变"""
        if self.current_track and self.current_track.filter_params:
            self.current_track.filter_params.cutoff_frequency = self.cutoff_spinbox.value()
            self.current_track.filter_params.resonance = self.resonance_spinbox.value()
            self.property_changed.emit(self.current_note, self.current_track)
    
    def on_delay_enabled_changed(self, enabled: bool):
        """延迟启用状态改变"""
        if self.current_track and self.current_track.delay_params:
            self.current_track.delay_params.enabled = enabled
            self.property_changed.emit(self.current_note, self.current_track)
    
    def on_delay_params_changed(self):
        """延迟参数改变"""
        if self.current_track and self.current_track.delay_params:
            self.current_track.delay_params.delay_time = self.delay_time_spinbox.value()
            self.current_track.delay_params.feedback = self.feedback_spinbox.value()
            self.current_track.delay_params.mix = self.mix_spinbox.value()
            self.property_changed.emit(self.current_note, self.current_track)
    
    def on_tremolo_enabled_changed(self, enabled: bool):
        """颤音启用状态改变"""
        if self.current_track and self.current_track.tremolo_params:
            self.current_track.tremolo_params.enabled = enabled
            self.property_changed.emit(self.current_note, self.current_track)
    ...
    def on_tremolo_params_changed(self):
        """颤音参数改变"""
        if self.current_track and self.current_track.tremolo_params:
            self.current_track.tremolo_params.rate = self.tremolo_rate_spinbox.value()
            self.current_track.tremolo_params.depth = self.tremolo_depth_spinbox.value()
            self.property_changed.emit(self.current_note, self.current_track)
