...
class GridSequenceWidget(QWidget):
    ...
    def refresh(self, force_full_refresh: bool = False):
        ...
        old_playhead_time = self.playhead_time
            ...
            try:
                ...
                if self.tracks:
                    required_height = len(self.tracks) * 60 + 60
                    self.view.setMinimumHeight(min(required_height, 600))
                    # 不设置最大高度，允许视图根据布局自动调整，从而启用滚动
    ...
    def _draw_tracks_and_blocks(self):
        ...
        for i, track in enumerate(self.tracks):
            ...
            if track.track_type == TrackType.DRUM_TRACK:
                sorted_events = sorted(track.drum_events, key=lambda e: e.start_beat)
                for event in sorted_events:
                    start_beats = round(event.start_beat * 4) / 4
                    x = 100 + start_beats * self.pixels_per_beat
    ...
    def draw_grid(self):
        ...
        # 计算需要绘制的拍数（从场景宽度计算）
        # 场景宽度 = 100（轨道标签宽度）+ 拍数 * pixels_per_beat
        max_beats = max(32, (scene_width - 100) / self.pixels_per_beat)
        max_beats = int(max_beats) + 4  # 多绘制一些，确保覆盖
    ...
    def on_view_mouse_press(self, event):
        """视图鼠标按下事件（用于播放线定位和拖动）"""
        # 检查是否点击在时间轴区域（音轨上方，Y < 50像素）或播放线上
        scene_pos = self.view.mapToScene(event.pos())
        
        # 检查是否点击在播放线附近（允许5像素的容差）
        beats_per_second = self.bpm / 60.0
        beat_position = self.playhead_time * beats_per_second
        playhead_x = 100 + beat_position * self.pixels_per_beat
        playhead_distance = abs(scene_pos.x() - playhead_x)
        
        # 如果点击在时间轴区域或播放线附近，开始拖动
        if (scene_pos.y() < 50 and scene_pos.x() >= 100) or (playhead_distance < 5 and scene_pos.y() < 200):
            self.is_dragging_playhead = True
            # 计算点击位置对应的时间
            x_offset = scene_pos.x() - 100
            if x_offset >= 0:
                beats = x_offset / self.pixels_per_beat
                # 吸附到1/4拍网格
                beats = round(beats * 4) / 4
                beats = max(0, beats)
                time = beats * 60.0 / self.bpm
    ...
    def on_wheel_event(self, event: QWheelEvent):
        ...
        if modifiers & Qt.AltModifier:
            ...
            if abs(new_scale - current_scale) > 0.001:  # 避免无意义的缩放
                ...
                angle_delta = event.angleDelta()
                pixel_delta = event.pixelDelta()
                print(f"Zoom: delta={delta}, angleDelta=({angle_delta.x()}, {angle_delta.y()}), pixelDelta=({pixel_delta.x() if pixel_delta else 0}, {pixel_delta.y() if pixel_delta else 0}), current={current_scale:.3f}, new={new_scale:.3f}, pixels={old_pixels:.1f}->{self.pixels_per_beat:.1f}")
                ...
                if abs(self.zoom_scale - new_scale) > 0.01:
                    print(f"WARNING: zoom_scale was reset! Expected {new_scale:.3f}, got {self.zoom_scale:.3f}")
                    self.zoom_scale = new_scale
                    self.pixels_per_beat = self.base_pixels_per_beat * self.zoom_scale
                ...
                # 如果缩放没有改变，打印调试信息（delta为0或其他原因）
                if abs(delta) < 1:
                    angle_delta = event.angleDelta()
                    pixel_delta = event.pixelDelta()
                    print(f"DEBUG: delta={delta}, angleDelta=({angle_delta.x()}, {angle_delta.y()}), pixelDelta=({pixel_delta.x() if pixel_delta else 0}, {pixel_delta.y() if pixel_delta else 0}), current={current_scale:.3f}, new={new_scale:.3f}")
    ...
    def on_horizontal_scroll(self, value):
        ...
        for label_item in self.track_label_items:
            if label_item and label_item.scene():
                ...
                if hasattr(label_item, 'original_y'):
                    label_item.setPos(fixed_x, label_item.original_y)
                else:
                    # 如果没有original_y，保持当前y坐标
                    current_y = label_item.pos().y()
                    label_item.original_y = current_y
                    label_item.setPos(fixed_x, current_y)
        ...
        for checkbox_proxy in self.checkbox_proxies:
            if checkbox_proxy and checkbox_proxy.scene():
                ...
                checkbox_proxy.setPos(5, current_y)
